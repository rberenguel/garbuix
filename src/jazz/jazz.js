...
const jazz = () => {
  ...
  window.addEventListener("keydown", jazzKeyboardListener);
  window.jazz = true;
};
...
const jazzKeyboardListener = (ev) => {
  if (ev.key === "Backspace") {
    if (note > 21) {
      note--;
      window.pianoNoteSampler.triggerAttackRelease(midiToNote(note), 0.5);
      return;
    }
  }
  ...
  keypressTimestamps.push(window.pianoNoteSampler.now());
  ...
  if (ev.key === "Enter" || ev.key == " ") {
    ...
    play = null;
    if (ev.key === "!") {
      window.drumNoteSampler.triggerAttackRelease("f0", 1);
    } else if (ev.key === ".") {
      window.drumNoteSampler.triggerAttackRelease("a0", 1);
    } else {
      window.drumNoteSampler.triggerAttackRelease("g0", 1);
    }
  } else {
